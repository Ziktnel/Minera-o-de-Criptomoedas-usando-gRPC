...
# -----------------------
class MinerServicer(grpcCalc_pb2_grpc.MinerServicer):
    def GetCurrentChallenge(self, request, context):
        with table_lock:
            current = table[-1]
            return grpcCalc_pb2.ChallengeInfo(
    ...
    def SubmitSolution(self, request, context):
        with table_lock:
            ...
            if request.transaction_id != current["TransactionID"]:
                return grpcCalc_pb2.SubmitResponse(
                    accepted=False,
    ...
    def Add(self, request, context):
        ...
        return grpcCalc_pb2.AdderResponse(sum=s)
...
def serve(port=50051):
    ...
    grpcCalc_pb2_grpc.add_MinerServicer_to_server(MinerServicer(), server)
